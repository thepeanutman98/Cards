 ...
 * @param  {Array<String>} extras                                        Extra cards to be loaded
...
function Card(a, x = 0, y = 0, e = 0.5, d = cards) {
  ...
  this.isIn = function(x, y) {
    switch (this.direction) {
      case 0:
        return x >= a.x && x <= a.cornerX && y >= a.y && y <= a.cornerY;
      case Math.PI * 0.5:
        return x <= a.x && x >= a.cornerX && y >= a.y && y <= a.cornerY;
      case Math.PI:
        return x <= a.x && x >= a.cornerX && y <= a.y && y >= a.cornerY;
  ...
  draw(s = 45) {
    if (this.direction) { // Detect if rotation is neccessary
      ...
      this.forEach(function(a, b) { // Iterates through each card in the Stack
        draw(this.flipped ? cards.empty : a.image, (b * s * this[0].size), 0, this[0].size); // Draws the card with proper distance apart
      }, this);
      ctx.restore();
    } else { // If rotation is not neccessary
      this.forEach(function(a, b) { // Iterates through each card in the Stack
        draw(this.flipped ? cards.empty : a.image, this.x + (b * s * this[0].sizze), this.y, this[0].size); // Draws the card with proper distance apart. this.x and this.y neccessary since Canvas origin is not being transalted
      }, this);
    }
  }
  /**
...
canvas.addEventListener("mousedown", function(e) {
  ...
  if (card) { // If the mouse is in a lone Card
    ...
    if (allLoneCards[0] != card) { // Detects of the card is topmost lone Card (the first in the allLoneCards array)
    ...
    var stack = allStacks.find((a) => (a.isIn(x, y)));
    console.log(stack);
    if (stack) {
      dragging = {
        type: "stack",
      ...
      stack.flipped = Boolean(e.button / 2);
      if (allStacks[0] != card) {
        allStacks.unshift(allStacks.splice(allStacks.indexOf(card), 1)[0]);
      }
      tick();
    }
    console.log(dragging);
  ...
  if (dragging) {
    if (e.movementX || e.movementY) {
      if (dragging.type == "card") {
        if (allLoneCards[0] != dragging.card) {
          allLoneCards.unshift(allLoneCards.splice(allLoneCards.indexOf(dragging.card), 1)[0]);
        }
      } else if (dragging.type == "stack") {
        if (allStacks[0] != dragging.card) {
          allStacks.unshift(allStacks.splice(allStacks.indexOf(card), 1)[0]);
        }
      }
      dragging.card.x += e.movementX;
      dragging.card.y += e.movementY;
      tick();
    }
  }
});
...
function tick() {
  clearCanvas();
  allPiles.slice().reverse().forEach(function(a) {
    a.draw();
  });
  allStacks.slice().reverse().forEach(function(a) {
    a.draw();
  });
  allLoneCards.slice().reverse().forEach(function(a) {
    a.draw();
  });
}
var allLoneCards = [new Card("ac", 77.5, 10), new Card("ad", 55, 10), new Card("ah", 32.5, 10), new Card("as", 10, 10)];
cards.empty.addEventListener("load", function() {
  tick();
}, {
  once: true
});
...
var fullDeck = [];
for (var i of ["a", 2, 3, 4, 5, 6, 7, 8, 9, 10, "j", "q", "k"]) {
  for (var j of ["h", "d", "c", "s"]) {
    fullDeck.push(new Card(i + j));
  }
}
